...
# Novas constantes para a galáxia
CENTRO_GALAXIA_X = LARGURA * 3.5  # Centro da galáxia no espaço virtual
CENTRO_GALAXIA_Y = ALTURA * 3.5
RAIO_GALAXIA = min(LARGURA, ALTURA) * 2  # Raio base da galáxia
NUM_BRACOS = 2  # Número de braços espirais
DENSIDADE_SPIRAL = 0.1  # Controla o quão "apertada" é a espiral
MASSA_BURACO_NEGRO = 100000  # Massa do buraco negro central
...
# Massa e limites para estrelas
MASSA_PLANETA = random.uniform(50, 200)  # Massa variável para planetas
LIMITE_INFERIOR_MASSA_ESTRELA = 1000  # Limite inferior da massa das estrelas
LIMITE_SUPERIOR_MASSA_ESTRELA = 3000  # Aumentado para mais variação
COR_ESTRELA = (150, 70, 8)  # Cor padrão atribuída às estrelas (RGB)

# Constantes relacionadas ao tamanho dos corpos celestes
RAIO_PLANETA = random.uniform(10, 30)  # Raio variável para planetas
RAIO_PROPORCIONALIDADE = 2  # Fator de proporcionalidade para calcular o raio
...
slider_estrelas = pygame_gui.elements.UIHorizontalSlider(
    ...
    start_value=NUM_ESTRELAS,
    value_range=(0, 2000),  # Changed from 1000 to 2000
    manager=gerenciador_interface,
)

slider_planetas = pygame_gui.elements.UIHorizontalSlider(
    relative_rect=pygame.Rect((10, 150), (200, 50)),
    start_value=NUM_PLANETAS,
    value_range=(0, 5000),  # Increased from 1000 to 5000
    manager=gerenciador_interface,
)
...
def gerar_posicao_espiral(indice, total_estrelas):
    """
    Gera uma posição seguindo um padrão espiral para criar braços galácticos.
    """
    # Escolhe um braço aleatório
    braco = random.randint(0, NUM_BRACOS - 1)
    
    # Distância do centro (aumenta gradualmente com o índice)
    r = random.uniform(0, RAIO_GALAXIA) * (indice / total_estrelas) ** 0.5
    
    # Ângulo base para o braço espiral
    angulo_base = (2 * math.pi * braco) / NUM_BRACOS
    
    # Ângulo da espiral (aumenta com a distância)
    angulo = angulo_base + (r * DENSIDADE_SPIRAL) + random.uniform(-0.2, 0.2)
    
    # Adiciona variação aleatória para dar volume à galáxia
    offset_z = random.uniform(-RAIO_GALAXIA * 0.1, RAIO_GALAXIA * 0.1)
    
    x = CENTRO_GALAXIA_X + r * math.cos(angulo)
    y = CENTRO_GALAXIA_Y + r * math.sin(angulo) + offset_z
    
    return x, y

def calcular_velocidade_orbital(x, y):
    """
    Calcula a velocidade orbital necessária para uma órbita estável.
    """
    dx = x - CENTRO_GALAXIA_X
    dy = y - CENTRO_GALAXIA_Y
    r = math.sqrt(dx*dx + dy*dy)
    
    if r < 1:  # Evita divisão por zero
        r = 1
    
    # Velocidade orbital baseada na massa central
    v_orbital = math.sqrt(G * MASSA_BURACO_NEGRO / r)
    
    # Componentes da velocidade perpendicular ao raio
    angulo = math.atan2(dy, dx) + math.pi/2  # Perpendicular ao raio
    vx = v_orbital * math.cos(angulo)
    vy = v_orbital * math.sin(angulo)
    
    # Adiciona pequena variação aleatória
    vx += random.uniform(-v_orbital*0.1, v_orbital*0.1)
    vy += random.uniform(-v_orbital*0.1, v_orbital*0.1)
...
def criar_estrelas(num_estrelas):
    """
    Cria estrelas em um padrão espiral galáctico.
    """
    estrelas = []
    
    # Adiciona o buraco negro central
    estrelas.append(Estrela(
        CENTRO_GALAXIA_X, CENTRO_GALAXIA_Y,
        0, 0,  # Velocidade zero para o centro
        MASSA_BURACO_NEGRO,
        50,  # Raio maior para visualização
        (255, 255, 255)  # Cor branca para o centro
    ))
    
    # Cria as demais estrelas
    for i in range(1, num_estrelas):
        x, y = gerar_posicao_espiral(i, num_estrelas)
        vx, vy = calcular_velocidade_orbital(x, y)
        
        # Varia a cor baseada na distância ao centro
        dist_centro = math.sqrt((x - CENTRO_GALAXIA_X)**2 + (y - CENTRO_GALAXIA_Y)**2)
        cor_base = int(255 * (1 - dist_centro/RAIO_GALAXIA))
        cor = (min(255, cor_base + 100), cor_base, max(0, cor_base - 50))
        
        massa = random.uniform(1000, 2000)
        raio = (massa ** (1/3)) * 2
        
        estrelas.append(Estrela(x, y, vx, vy, massa, raio, cor))
...
def criar_planetas(num_planetas):
    """
    Cria planetas em órbitas aleatórias dentro da galáxia com propriedades variadas.
    """
    planetas = []
    for _ in range(num_planetas):
        # Variar a distância do centro para distribuir melhor os planetas
        r = random.uniform(0, RAIO_GALAXIA * 0.8)  # Increased from 0.5 to 0.8
        angulo = random.uniform(0, 2 * math.pi)
        
        x = CENTRO_GALAXIA_X + r * math.cos(angulo)
        y = CENTRO_GALAXIA_Y + r * math.sin(angulo)
        
        # Calculate orbital velocities with some random variation
        vx, vy = calcular_velocidade_orbital(x, y)
        vx *= random.uniform(0.8, 1.2)  # Add 20% variation
        vy *= random.uniform(0.8, 1.2)  # Add 20% variation
        
        # Randomize planet properties
        massa = random.uniform(50, 200)
        raio = random.uniform(10, 30)
        
        # More varied colors for planets
        cor = (
            random.uniform(0, 100),  # Red component
            random.uniform(50, 255), # Green component
            random.uniform(50, 255)  # Blue component
        )
